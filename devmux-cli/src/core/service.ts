...
export async function ensureService(
  ...
  if (!service) {
    throw new Error(`Unknown service: ${serviceName}. Available services: ${Object.keys(config.services).join(", ")}`);
  }
...
export async function restartService(
  ...
  await stopService(config, serviceName, { killPorts: options.killPorts, quiet: options.quiet });
