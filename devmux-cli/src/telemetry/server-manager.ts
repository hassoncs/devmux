...
const PID_FILE = join(homedir(), ".devmux", "telemetry-server.pid");
...
export function startServer(options: { port?: number; host?: string } = {}): boolean {
  ...
  if (host === "0.0.0.0" || host === "::") {
    console.error(
      "⚠️ Telemetry server binding to all interfaces — it has no authentication; anyone on your network can send log events.",
    );
  }
