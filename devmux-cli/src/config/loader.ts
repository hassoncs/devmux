...
function resolveLoadedConfig(
  ...
  const resolvedSessionPrefix = config.sessionPrefix ?? `devmux-${config.project}`;
...
function validateConfig(config: unknown): config is DevMuxConfig {
  ...
  for (const [name, service] of Object.entries(c.services as Record<string, unknown>)) {
    ...
    if (typeof s.command !== "string" || s.command.length === 0) {
      throw new Error(`Invalid service "${name}": "command" must be a non-empty string`);
    }
    if (s.cwd !== undefined && typeof s.cwd !== "string") {
      throw new Error(`Invalid service "${name}": "cwd" must be a string`);
    }
    if (s.health !== undefined) {
      const h = s.health as Record<string, unknown>;
      if (!h || typeof h !== "object" || !["port", "http", "none"].includes(h.type as string)) {
        throw new Error(`Invalid service "${name}": health.type must be "port", "http", or "none"`);
      }
    }
  }
 ...
 * tmux treats ':' and '.' as delimiters (session:window.pane), and session
 * names sourced from config flow into subprocess invocations, so only a
 * conservative allowlist of characters is kept; everything else becomes '-'.
 */
