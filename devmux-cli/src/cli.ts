...
const stop = defineCommand({
  ...
  async run({ args }) {
    ...
    if (serviceName === "all") {
      await stopAllServices(config, { killPorts: args.force });
      ...
      await stopService(config, serviceName, { killPorts: args.force });
    }
  },
});
...
const proxyStart = defineCommand({
  ...
  async run() {
    const result = await proxySystem.tryStartManagedProxy({
      checkAvailable: () => caddy.isAvailable(),
      getReport: () => proxySystem.getProxyDoctorReport(false),
    });

    if (result.available) {
      console.log(result.attempted ? "Proxy: Started" : "Proxy: Running");
      return;
    }

    const report = proxySystem.getProxyDoctorReport(false);
    ...
    console.log("");
    console.log("If it is already installed but stopped, start it with:");
...
const proxyTeardown = defineCommand({
  meta: {
    name: "teardown",
    description: "Remove the managed Caddy LaunchDaemon, binary, and config",
  },
  args: {
    apply: {
      type: "boolean",
      description: "Execute the removal commands (requires sudo)",
    },
  },
  async run({ args }) {
    if (args.apply && (typeof process.getuid !== "function" || process.getuid() !== 0)) {
      console.error("❌ --apply must be run as root.");
      console.error("   Run: sudo devmux proxy teardown --apply");
      process.exit(1);
    }
    await proxySystem.printTeardownInstructions(Boolean(args.apply));
  },
});
