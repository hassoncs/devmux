...
describe("CaddyManager", () => {
	...
	describe("getServerName", () => {
		...
		it("throws when Caddy has no HTTP servers configured", async () => {
			mockFetch.mockResolvedValueOnce(makeResponse(200, {}));
			await expect(manager.getServerName()).rejects.toThrow("no configured HTTP servers");
		});

		it("throws on non-ok response", async () => {
			mockFetch.mockResolvedValueOnce(makeResponse(500, {}));
			await expect(manager.getServerName()).rejects.toThrow("Caddy API error getting servers");
		});
	});
	...
	describe("registerRoute", () => {
		it("sends DELETE then POST in sequence with correct URLs and body", async () => {
			mockFetch
			...
			expect(deleteUrl).toBe("http://localhost:2019/id/devmux_api_myapp_localhost");
			expect(deleteInit.method).toBe("DELETE");

			const [postUrl, postInit] = mockFetch.mock.calls[2] as [string, RequestInit];
			expect(postUrl).toBe(
			...
			expect(body["@id"]).toBe("devmux_api_myapp_localhost");
			expect((body.match as Array<{ host: string[] }>)[0].host[0]).toBe("api.myapp.localhost");
		});
		...
		it("throws when POST fails", async () => {
			...
			await expect(manager.registerRoute("bad.localhost", 9999)).rejects.toThrow(
				'Failed to register Caddy route for "bad.localhost"',
			);
		});
	});
	...
	describe("deregisterRoute", () => {
		...
		it("swallows 404 response (idempotent, no throw)", async () => {
			...
			await expect(manager.deregisterRoute("gone.localhost")).resolves.toBeUndefined();
		});
	...
	describe("listRoutes", () => {
		...
		it("throws on non-ok response", async () => {
			...
			await expect(manager.listRoutes()).rejects.toThrow("Failed to list Caddy routes");
		});
	});
});
