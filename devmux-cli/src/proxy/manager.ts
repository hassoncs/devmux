...
export async function deregisterRoute(config: ResolvedConfig, serviceName: string): Promise<void> {
	const hostname = getServiceHostname(config, serviceName);
...
export async function ensureProxyRunning(config: ResolvedConfig): Promise<void> {
	...
	if (!(await caddy.isAvailable())) {
		throw new Error("Caddy is not running. Start it with: brew services start caddy");
	}
}
