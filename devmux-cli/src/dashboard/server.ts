...
function tryListen(server: Server, port: number, host: string, maxRetries: number): Promise<number> {
...
export async function startDashboard(options: DashboardOptions = {}): Promise<DashboardResult> {
	...
	if (host === "0.0.0.0" || host === "::") {
		process.stderr.write(
			`[devmux-dashboard] WARNING: binding to ${host} — dashboard is reachable from the network.\n`
		);
	}
	...
	actualPort = await tryListen(server, basePort, host, MAX_PORT_RETRIES);
