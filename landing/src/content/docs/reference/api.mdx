...
<Aside type="caution">
The programmatic API signatures documented here are illustrative. For the precise current signatures, import from `@chriscode/devmux` and check the TypeScript types directly. The CLI is the stable interface; the programmatic API may change between minor versions.
</Aside>
