...
1. **Initialize configuration**
   ...
   ```bash
   npx devmux discover turbo > devmux.config.json
   ```

   This outputs a `devmux.config.json` based on your `turbo.json` persistent tasks. Review and edit the result to add health checks.
...
3. **Start a service**
   ...
   This:
   - Creates a tmux session named `devmux-myapp-api`
