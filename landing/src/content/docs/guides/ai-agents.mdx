...
|---------|-------------------|------|
| API     | `devmux-myapp-api`   | 8787 |
| Web     | `devmux-myapp-web`   | 3000 |
...
\`\`\`bash
# API Server
tmux new-session -d -s devmux-myapp-api -c api "pnpm dev"
\`\`\`
...
$ devmux ensure api
Created devmux-myapp-api, running on port 8787

# Later, AI agent needs the API
$ devmux ensure api
devmux-myapp-api already running on port 8787 (reusing)
```
