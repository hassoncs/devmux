...
- **Version:** see `devmux-cli/package.json`
...
│   └── index.ts      # Exports
├── proxy/            # Portless .localhost proxy
│   ├── caddy.ts      # Caddy admin API client
│   ├── manager.ts    # Port allocation, route lifecycle
...
- **Session naming convention** - `{prefix}-{project}-{service}` (e.g., `devmux-myapp-api`)
...
- Verify session naming: `devmux-{project}-{service}`
...
4. Writes to `~/.devmux/queue.jsonl`
...
JSONL file at `~/.devmux/queue.jsonl`:
```json
{
  "id": "uuid",
  "service": "api",
...
- Requires one-time system setup: run `sudo devmux proxy setup --apply` to install Caddy as a LaunchDaemon (macOS only) listening on port 80; Caddy dynamically routes to each service via its admin API
...
- Session names follow pattern `devmux-{project}-{service}`
