...
| **List project ports**     | `devmux ports`         | Shows the current repo's configured service ports from local devmux config.                   |
| **Machine-readable ports** | `devmux ports --json`  | Emits JSON so higher-level tooling can inspect the current repo without parsing config files. |
| **Plain port lines**       | `devmux ports --plain` | Emits tab-separated rows for shell pipelines.                                                 |
...
### 2b. The "Port Planning" Rule

Before changing ports in the current repo, inspect the repo through `devmux`, not by manually parsing config files.
...
Workspace-wide conflict checks belong in higher-level tooling that enumerates projects and calls `devmux ports --json` per repo.
...
Sessions follow the pattern: `devmux-{project}-{service}`.

- Example: `devmux-waypoint-api`
- You can use standard `tmux` commands if needed: `tmux capture-pane -t devmux-waypoint-api -p`
- OpenCode users may set `"sessionPrefix": "omo-myproject"` in their config — in that case session names use `omo-` instead. Check the project's `devmux.config.json` or run `devmux status` to see the actual session names.
