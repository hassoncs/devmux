...
export class TelemetryServer {
  ...
  constructor(options: TelemetryServerOptions = {}) {
    ...
    if (this.host === "0.0.0.0" || this.host === "::") {
      process.stderr.write(
        `[telemetry-server] WARNING: binding to ${this.host} — server is reachable from the network and has no authentication.\n`
      );
    }
  ...
  private handleConnection(ws: WebSocket, req: IncomingMessage): void {
    ...
    if (origin !== undefined) {
      // Browser client — enforce origin allowlist
      if (!this.isAllowedOrigin(origin)) {
        console.error(
          `[telemetry-server] Rejected connection from disallowed origin: ${origin}`
        );
        ws.close(4403, "Forbidden: origin not allowed");
        return;
      }
    }
    // No Origin header — native/RN client, allow through
