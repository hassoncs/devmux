...
import { appendFileSync, chmodSync, existsSync, mkdirSync, statSync, readFileSync, writeFileSync } from "node:fs";
...
const DEFAULT_OUTPUT_DIR = join(homedir(), ".devmux");
...
export class Router {
  ...
  private createTmuxSession(sessionName: string): boolean {
    try {
      execFileSync(
        "tmux",
        ["new-session", "-d", "-s", sessionName, "-x", "200", "-y", "50", "cat"],
        { stdio: ["pipe", "pipe", "pipe"] }
      );
      execFileSync(
        "tmux",
  ...
  private sendToTmux(sessionName: string, content: string): void {
    try {
      // argv form: content is never interpreted by a shell
      execFileSync("tmux", ["send-keys", "-t", sessionName, content, "Enter"], {
        stdio: ["pipe", "pipe", "pipe"],
      });
    } catch {
