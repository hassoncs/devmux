...
2. **Session conflicts**: All worktrees try to create the same tmux session names (e.g., `devmux-myapp-api`)
...
|-------------|---------|--------------|
| (empty) | api | `devmux-myapp-api` |
| `feature-x` | api | `devmux-myapp-feature-x-api` |
| `agent-1` | worker | `devmux-myapp-agent-1-worker` |
...
**Behavior:**
- Without `DEVMUX_INSTANCE_ID`: Port 8787, session `devmux-myapp-api`
- With `DEVMUX_INSTANCE_ID=feature-x`: Port 9210, session `devmux-myapp-feature-x-api`
